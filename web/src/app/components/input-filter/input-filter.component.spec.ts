...
describe('InputFilterComponent', () => {
  ...
  it('should not show the tag list on init render', () => {
    const tagListDebugElement: DebugElement = fixture.debugElement.query(By.css('.input-filter-tags'));
  ...
  it('should show the tag list if down arrow icon clicked', () => {
    ...
    const downArrowIconDebugElement: DebugElement = fixture.debugElement.query(By.css('.down-icon'));
    downArrowIconDebugElement.triggerEventHandler('click', null);
    expect(component.showTagList).toBe(true);
    fixture.detectChanges();
    const tagListElement: HTMLElement = fixture.debugElement.query(By.css('.input-filter-tags')).nativeElement;
  ...
  it('should show the user text if there are no filters', () => {
    ...
    const userTextDebugElement: DebugElement = fixtureDebugElement.query(By.css('.input-filter-empty'));
    const userTextNativeElement: HTMLElement = userTextDebugElement.nativeElement;
    expect(userTextNativeElement.textContent).toMatch(/No current filters/i);
  });

  it('should show the tags if there are filters', () => {
    const labelFilterService: LabelFilterService = TestBed.get(LabelFilterService);
    ...
    expect(tagDebugElements[0].nativeElement.textContent).toMatch(/test1:filter1/i);
    expect(tagDebugElements[1].nativeElement.textContent).toMatch(/test2:filter2/i);
    expect(tagDebugElements[2].nativeElement.textContent).toMatch(/test3:filter3/i);
  });
  ...
  it('should be able to enter a tag through the input', async(() => {
    ...
    labelFilterService.decodeFilter = jasmine.createSpy('decodeFilter').and.returnValue({ key: 'test1', value: 'filter1' });
    ...
    fixture.whenStable().then(() => {
      inputNativeElement = fixture.debugElement.query(By.css('.text-input')).nativeElement;
