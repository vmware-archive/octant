...
@Component({
  selector: 'app-logs',
  styles: [".highlight {color: #ffdf5d} .highlight-selected {background: blue}"],
...
export class LogsComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  public filterFunction(logs: LogEntry[]): LogEntry[] {
    if (this.showOnlyFiltered) {
      if (this.shouldDisplayTimestamp) {
        return logs.filter(log => log.message.match(new RegExp(this.filterText, "gi"))
          || formatDate(log.timestamp, "long", "en-US")
            .match(new RegExp(this.filterText, "gi")));
      }
      return logs.filter(log => log.message.match(new RegExp(this.filterText, "gi")));
    }
  ...
  scrollToHighlight(scrollBy: number) {
    ...
    if (this.getHighlightedElement(this.currentSelection + scrollBy)) {
      ...
      const nextSelection: HTMLElement = this.getHighlightedElement(this.currentSelection);
      const { clientHeight, offsetTop, scrollTop } = this.scrollTarget.nativeElement;
  ...
  removeHighlightSelection(): HTMLElement {
    const element: HTMLElement = this.getHighlightedElement(this.currentSelection);
    if (element) {
