...
import { NotifierService, NotifierSignal, NotifierSignalType } from './notifier.service';
...
describe('NotifierService', () => {
  ...
  it('should be able to add signals', () => {
    ...
    const loadingSignalID = service.pushSignal(NotifierSignalType.LOADING, true);
    const errorSignalID = service.pushSignal(NotifierSignalType.ERROR, 'You are doing it wrong');
  ...
  it('should be able to remove signals', () => {
    ...
    const warningSignalID = service.pushSignal(NotifierSignalType.WARNING, 'nope nope nope');
    const loadingSignalID = service.pushSignal(NotifierSignalType.LOADING, false);
    const errorSignalID = service.pushSignal(NotifierSignalType.ERROR, 'No more worky');
  ...
  it('should be able to create a signal session', () => {
    ...
    const loadingSignalID = session.pushSignal(NotifierSignalType.LOADING, false);
    const errorSignalID = session.pushSignal(NotifierSignalType.ERROR, 'No more worky');
